...
- In useEffect, fetch rules from `/api/rules/${documentType}` when documentType changes
- Use fetch() to call the backend API endpoint
- Parse JSON response and dispatch LOAD action
...
- save() - calls PUT /api/rules/{document_type} with current rules
- reset() - calls POST /api/rules/{document_type}/reset then reloads
- reload() - refetches from API
...
cd frontend && npm run build
  ...
  <done>Settings accessible from main app, TypeScript compiles without errors</done>
</task>
