...
curl http://localhost:8000/api/rules/factsheet
curl http://localhost:8000/api/rules/publication
  </verify>
...
If .gitignore doesn't exist, create it with common Python ignores plus the user_config rule.
  ...
  <verify>
Check that user_config/*.yaml would be ignored:
cd backend && git check-ignore user_config/test.yaml || echo "File would be tracked"
  </verify>
